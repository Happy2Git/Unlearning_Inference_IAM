...
def get_scores(args_input):
    ...
    for idx,shadow_list in enumerate(shadow_lists):
        ...
        for loop in range(args.LOOP):
            ...
            num_pos = Member_flags[:, loop].sum()
            num_neg = len(Member_flags[:, loop]) - num_pos
            pos_weight = num_neg / num_pos  # weight for positive class
            weights = torch.tensor( np.where(Member_flags[:, loop] == 1, pos_weight, 1.0))
            # print(f'dtype {type(Scores[key][:,loop])} {Scores[key][:2,loop]} {type(Member_flags[:,loop])} {Member_flags[:2,loop]}')
            bce_loss = F.binary_cross_entropy(torch.tensor(Scores[key][:,loop], dtype=torch.float32), 
                                              torch.tensor(Member_flags[:,loop], dtype=torch.float32),
                                              weight=weights )
            mean_1 = Scores[key][:,loop][Member_flags[:,loop]==1].mean()
            std_1 = Scores[key][:,loop][Member_flags[:,loop]==1].std()
            max_1 = Scores[key][:,loop][Member_flags[:,loop]==1].max()
            min_1 = Scores[key][:,loop][Member_flags[:,loop]==1].min()
            mean_0 = Scores[key][:,loop][Member_flags[:,loop]==0].mean()
            std_0 = Scores[key][:,loop][Member_flags[:,loop]==0].std()
            max_0 = Scores[key][:,loop][Member_flags[:,loop]==0].max()
            min_0 = Scores[key][:,loop][Member_flags[:,loop]==0].min()
            Metric_values[class_idx_]['auc'] = auc_score
            Metric_values[class_idx_]['bce_loss'] = bce_loss
            Metric_values[class_idx_]['mean-1'] = mean_1
            Metric_values[class_idx_]['std-1'] = std_1
            Metric_values[class_idx_]['max-1'] = max_1
            Metric_values[class_idx_]['min-1'] = min_1
            # last 10% average score for label-1
            idxs = np.argsort(Scores[key][:,loop][Member_flags[:,loop]==1])
            Metric_values[class_idx_]['mean-1_last'] = Scores[key][:,loop][Member_flags[:,loop]==1][idxs[:int(len(idxs)*0.1)]].mean()
            
            Metric_values[class_idx_]['mean-0'] = mean_0
            Metric_values[class_idx_]['std-0'] = std_0
            Metric_values[class_idx_]['max-0'] = max_0
            Metric_values[class_idx_]['min-0'] = min_0
            # last 10% average score for label-0
            idxs = np.argsort(Scores[key][:,loop][Member_flags[:,loop]==0])
            Metric_values[class_idx_]['mean-0_last'] = Scores[key][:,loop][Member_flags[:,loop]==0][idxs[-int(len(idxs)*0.1):]].mean()
            


        Member_flags = Member_flags.flatten()
        ...
        # overall metrics
        auc_score, _, _, _, _ = auc_extended(
                    Member_flags, Scores[key], verbose=False)
        num_pos = Member_flags.sum()
        num_neg = len(Member_flags) - num_pos
        pos_weight = num_neg / num_pos  # weight for positive class
        weights = torch.tensor( np.where(Member_flags == 1, pos_weight, 1.0))
        bce_loss = F.binary_cross_entropy(torch.tensor(Scores[key], dtype=torch.float32), 
                                          torch.tensor(Member_flags, dtype=torch.float32),
                                          weight=weights )
        mean_1 = Scores[key][Member_flags==1].mean()
        std_1 = Scores[key][Member_flags==1].std()
        max_1 = Scores[key][Member_flags==1].max()
        min_1 = Scores[key][Member_flags==1].min()
        # last 10% average score for label-1
        idxs = np.argsort(Scores[key][Member_flags==1])
        mean_1_last = Scores[key][Member_flags==1][idxs[:int(len(idxs)*0.1)]].mean()

        mean_0 = Scores[key][Member_flags==0].mean()
        std_0 = Scores[key][Member_flags==0].std()
        max_0 = Scores[key][Member_flags==0].max()
        min_0 = Scores[key][Member_flags==0].min()
        # last 10% average score for label-0
        idxs = np.argsort(Scores[key][Member_flags==0])
        mean_0_last = Scores[key][Member_flags==0][idxs[-int(len(idxs)*0.1):]].mean()

        c_std1 = np.std([Metric_values[class_idx_]['mean-1'] for class_idx_ in range(args.CLASS_init, args.CLASS_init+args.LOOP)])
        c_std0 = np.std([Metric_values[class_idx_]['mean-0'] for class_idx_ in range(args.CLASS_init, args.CLASS_init+args.LOOP)])
        b_std = np.std([Metric_values[class_idx_]['bce_loss'] for class_idx_ in range(args.CLASS_init, args.CLASS_init+args.LOOP)])
        Metric_values['overall']['auc'] = auc_score
        Metric_values['overall']['bce_loss'] = bce_loss
        Metric_values['overall']['mean-1'] = mean_1
        Metric_values['overall']['std-1'] = std_1
        Metric_values['overall']['max-1'] = max_1
        Metric_values['overall']['min-1'] = min_1
        Metric_values['overall']['mean-1_last'] = mean_1_last        

        Metric_values['overall']['mean-0'] = mean_0
        Metric_values['overall']['std-0'] = std_0
        Metric_values['overall']['max-0'] = max_0
        Metric_values['overall']['min-0'] = min_0
        Metric_values['overall']['mean-0_last'] = mean_0_last

        Metric_values['overall']['b-std'] = b_std
        Metric_values['overall']['c-std1'] = c_std1
        Metric_values['overall']['c-std0'] = c_std0

        random_guess = 0.5 * np.ones_like(Member_flags)
        auc_score, _, _, _, _ = auc_extended(
                    Member_flags, random_guess, verbose=False)
        bce_loss = F.binary_cross_entropy(torch.tensor(random_guess, dtype=torch.float32),
                                            torch.tensor(Member_flags, dtype=torch.float32),
                                            weight=weights )
        mean_1 = random_guess[Member_flags==1].mean()
        std_1 = random_guess[Member_flags==1].std()
        max_1 = random_guess[Member_flags==1].max()
        min_1 = random_guess[Member_flags==1].min()
        # last 10% average score for label-1
        idxs = np.argsort(random_guess[Member_flags==1])
        mean_1_last = random_guess[Member_flags==1][idxs[:int(len(idxs)*0.1)]].mean()

        mean_0 = random_guess[Member_flags==0].mean()
        std_0 = random_guess[Member_flags==0].std()
        max_0 = random_guess[Member_flags==0].max()
        min_0 = random_guess[Member_flags==0].min()
        # last 10% average score for label-0
        idxs = np.argsort(random_guess[Member_flags==0])
        mean_0_last = random_guess[Member_flags==0][idxs[-int(len(idxs)*0.1):]].mean()

        Metric_values['random'] = {}
        Metric_values['random']['auc'] = auc_score
        Metric_values['random']['bce_loss'] = bce_loss
        Metric_values['random']['mean-1'] = mean_1
        Metric_values['random']['std-1'] = std_1
        Metric_values['random']['max-1'] = max_1
        Metric_values['random']['min-1'] = min_1
        Metric_values['random']['mean-1_last'] = mean_1_last
        Metric_values['random']['mean-0'] = mean_0
        Metric_values['random']['std-0'] = std_0
        Metric_values['random']['max-0'] = max_0
        Metric_values['random']['min-0'] = min_0
        Metric_values['random']['mean-0_last'] = mean_0_last

        Metrics_print = {}
        for key in Metric_values.keys():
            if key == 'overall':
                Metrics_print[key] = [Metric_values[key]['auc'], Metric_values[key]['bce_loss'], Metric_values[key]['mean-1'], 
                                      Metric_values[key]['std-1'], Metric_values[key]['max-1'], Metric_values[key]['min-1'], Metric_values[key]['mean-1_last'],
                                      Metric_values[key]['mean-0'], Metric_values[key]['std-0'], Metric_values[key]['max-0'], Metric_values[key]['min-0'], Metric_values[key]['mean-0_last'],
                                      Metric_values[key]['b-std'], Metric_values[key]['c-std1'], Metric_values[key]['c-std0']]
            else:
                Metrics_print[key] = [Metric_values[key]['auc'], Metric_values[key]['bce_loss'], Metric_values[key]['mean-1'], Metric_values[key]['mean-1_last'],
                                      Metric_values[key]['std-1'],  Metric_values[key]['max-1'], Metric_values[key]['min-1'],  Metric_values[key]['mean-0_last'],
                                      Metric_values[key]['mean-0'], Metric_values[key]['std-0'], Metric_values[key]['max-0'], Metric_values[key]['min-0']]

        print('\nstart_print')
        print(args)
        print( Scores.keys())

        print(f'{"Metric values ":<30}', f'{"auc":<6}', f'{"bce_l":<6}',
            f'{"mean-1":<6}', f'{"std-1":<6}', f'{"max-1":<6}', f'{"min-1":<6}', f'{"last-1":<6}', f'{"mean-0":<6}', 
            f'{"std-0":<6}', f'{"max-0":<6}', f'{"min-0":<6}', f'{"last-0":<6}', '{"b-std":<6}', f'{"c-std1":<6}', f'{"c-std0":<6}')

        for key in Metrics_print.keys():
                print(f'{key:<30}', " ".join(f"{x:.4f}" for x in Metrics_print[key]))
        ...
        save_name = records_folder + f'class_{args.SUFFIX}_scores_{SEED_init}_{args.LOOP}_ref{args.model_numbs}.pth'
        torch.save(Scores, save_name)
        save_name = records_folder + f'class_{args.SUFFIX}_unlearn_flags_{SEED_init}_{args.LOOP}_ref{args.model_numbs}.pth'
        torch.save(Unlearn_flags, save_name)
        save_name = records_folder + f'class_{args.SUFFIX}_metric_{SEED_init}_{args.LOOP}_ref{args.model_numbs}.pth'
        torch.save(Metric_values, save_name)
...
def parse_args():
    ...
    parser.add_argument('--unlearn_type', type=str, default='one_class', 
    ...
    # parser.add_argument('--metrics', nargs='+', default=['p_Unleak', 'p_LiRA', 'p_update_LiRA', 'p_EMIA', 'p_EMIA_p', 'p_RMIA', 'p_interapprox'],
...
if __name__ == "__main__":
    ...
    if None == args_input.dataname and 'all' == args_input.unlearn_method:
        for dataname in ['cifar10', 'cinic10',  'cifar100', 'purchase']:
        # for dataname in ['cifar100']:
            args_input.dataname = dataname
            for unlearn_method in ['retrain','finetune', 'ssd', 'fisher', 'forsaken', 'l_codec', 'ascent', 'boundary_expanding']:
                args_input.unlearn_method = unlearn_method
                get_scores(args_input)
    elif 'all' == args_input.unlearn_method:
        for unlearn_method in ['retrain','finetune', 'ssd', 'fisher', 'forsaken', 'l_codec', 'ascent', 'boundary_expanding']:
            args_input.unlearn_method = unlearn_method
            get_scores(args_input)
    elif None == args_input.dataname:
        for dataname in ['cifar10', 'cinic10',  'cifar100', 'purchase']:
            args_input.dataname = dataname
            get_scores(args_input)
    else:
