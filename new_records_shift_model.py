...
def audit_nonmem_mono(args, DEVICE=torch.device("cpu"), SEED=42, batch_size=1024):
    ...
    if 'p_Unleak' in args.metrics:
        ...
        attack_model = train_attack_model(
            args, model_list=args.SHADOW_IDXs,  shadow_path= args.shadow_folder + f'{args.dataname}', 
            DEVICE=DEVICE, arch=args.model_arch)
...
def load_records(records_folder, args, batch_size=1024, DEVICE='cpu', SEED='42'):
    ...
    target_shadow_logits_path = args.records_folder + f'share_ori/{dataname}_target_shadows_arch{args.shadow_arch}_logits.pth' # #number x #shadow x #class
    ...
    ref_shadow_logits_path = records_folder + f'{dataname}_ref_shadow_arch{args.shadow_arch}_logits.pth' # #number x #ref x #class
    ...
    if RELOAD_SHADOW:
        ...
        for shadow_idx in range(shadow_numbs):
            load_path = args.shadow_folder + f'{dataname}/shadow_model_{shadow_idx}arch_{args.shadow_arch}.pth'
            ...
            if args.shadow_arch == 'ResNet18':
                shadow_model = ResNet18(num_classes=100)
            elif args.shadow_arch == 'ResNet34':
                shadow_model = ResNet34(num_classes=100)
            elif args.shadow_arch == 'ResNet50':
                shadow_model = ResNet50(num_classes=100)
                epochs_set = 150
            elif args.shadow_arch == 'vgg16':
                shadow_model = vgg16(weights=None)
                shadow_model.classifier[6] = torch.nn.Linear(in_features=4096, out_features=100)
                lr = 0.01
            elif args.shadow_arch =='vgg11':
                shadow_model = vgg11(weights=None)
                shadow_model.classifier[6] = torch.nn.Linear(in_features=4096, out_features=100)
                lr = 0.01
            elif args.shadow_arch == 'shufflenet': # RuntimeError: Calculated padded input size per channel: (1 x 1). Kernel size: (3 x 3). Kernel size can't be greater than actual input size
...
def fetch_data_model(args,  verbose=False, SEED = 42, DEVICE='cpu'):
    ...
    if args.dataname == 'cifar100' and args.model_arch == 'shufflenet':
    ...
    elif args.dataname == 'cifar100' and args.model_arch == 'swin_t':
        def get_cifar_transforms(train=True):
            if train:
                transform = transforms.Compose([
                    transforms.Resize(224),  # Resize to ViT expected size
    ...
    # split dataname
    init_train_loader, retain_loader, forget_loader, val_loader, test_loader, shadow_set, cut_train_set, unlearn_flags, val_set  = get_data_loaders(dataname, unlearn_type=unlearn_type, 
                                                                                                                                                    forget_class=forget_class, forget_size=forget_size, SEED=SEED,
    ...
    shadow_training(shadow_set, dataname, ratio=0.8, ORIGIN=True, shadow_nums=args.model_numbs, shadow_path=shadow_path, DEVICE=DEVICE, VERBOSE=verbose, ARCH=True, arch_name=args.model_arch)

    if dataname == 'cifar10' or dataname == 'cifar100'  or dataname == 'cinic10':
        weights_pretrained, _ = load_pretrain_weights_shift(DEVICE, TRAIN_FROM_SCRATCH=True, RETRAIN=False, dataname=dataname,
                                                train_loader=init_train_loader, test_loader=val_loader, 
                                                checkpoints_folder='LIRA_checkpoints', SEED=SEED,
                                                arch_name=args.model_arch)
        # load model with pre-trained weights
        if args.model_arch == 'ResNet18':
            ...
            lr = 0.01
        elif args.model_arch == 'vgg11':
            model = vgg11(weights=None)
            model.classifier[6] = torch.nn.Linear(in_features=4096, out_features=100)
            lr = 0.01
        elif args.model_arch == 'shufflenet': # RuntimeError: Calculated padded input size per channel: (1 x 1). Kernel size: (3 x 3). Kernel size can't be greater than actual input size
            ...
            model.classifier = torch.nn.Linear(in_features=1024, out_features=100)
        elif args.model_arch == 'MobileNetV2':
            model = mobilenet_v2(weights=None)
            model.classifier[1] = torch.nn.Linear(in_features=1280, out_features=100)
            epochs_set = 300
        elif args.model_arch == 'EfficientNet':
            model = efficientnet_b0(weights=None)
            model._fc = torch.nn.Linear(in_features=1280, out_features=100)
            epochs_set = 300
        elif args.model_arch == 'swin_t':
            model = swin_t(weights=None)  # Initialize without pretrained weights
            # Modify head for CIFAR100 (100 classes)
    ...
    if unlearn_method == 'retrain':
        if dataname == 'cifar10' or dataname == 'cifar100' or dataname == 'cinic10':
            weights_rt_pretrained, _ = load_pretrain_weights_shift(DEVICE, TRAIN_FROM_SCRATCH=True, RETRAIN=True, dataname=dataname,
                                                            train_loader=retain_loader, test_loader=val_loader, 
                                                            checkpoints_folder='LIRA_checkpoints', SUFFIX=SUFFIX, 
                                                            SEED=SEED, arch_name=args.model_arch)
            # load model with pre-trained weights
            if args.model_arch == 'ResNet18':
                ul_model = ResNet18(num_classes=100)
            elif args.model_arch == 'ResNet34':
                ul_model = ResNet34(num_classes=100)
            elif args.model_arch == 'ResNet50':
                ul_model = ResNet50(num_classes=100)
                epochs_set = 150
            elif args.model_arch == 'vgg16':
                ul_model = vgg16(weights=None)
                ul_model.classifier[6] = torch.nn.Linear(in_features=4096, out_features=100)
                lr = 0.01
            elif args.model_arch == 'vgg11':
                ul_model = vgg11(weights=None)
                ul_model.classifier[6] = torch.nn.Linear(in_features=4096, out_features=100)
                lr = 0.01
            elif args.model_arch == 'shufflenet': # RuntimeError: Calculated padded input size per channel: (1 x 1). Kernel size: (3 x 3). Kernel size can't be greater than actual input size
                ul_model = shufflenet_v2_x1_0(weights=None)
                epochs_set = 100
                # Modify classifier for CIFAR100
                ul_model.fc = torch.nn.Linear(ul_model.fc.in_features, 100)
            elif args.model_arch == 'DenseNet':
                ul_model = densenet121(weights=None)
                ul_model.classifier = torch.nn.Linear(in_features=1024, out_features=100)
            elif args.model_arch == 'MobileNetV2':
                ul_model = mobilenet_v2(weights=None)
                ul_model.classifier[1] = torch.nn.Linear(in_features=1280, out_features=100)
                epochs_set = 300
            elif args.model_arch == 'EfficientNet':
                ul_model = efficientnet_b0(weights=None)
                ul_model._fc = torch.nn.Linear(in_features=1280, out_features=100)
                epochs_set = 300
            elif args.model_arch == 'swin_t':
                ul_model = swin_t(weights=None)  # Initialize without pretrained weights
                # Modify head for CIFAR100 (100 classes)
                ul_model.head = torch.nn.Linear(ul_model.head.in_features, 100)
            print(ul_model)
            print('*********************')
            # print(weights_rt_pretrained)
...
def create_shadow_lists(total_models=128, model_nums=1):
    ...
    group_nums = min(total_models // model_nums, 1)
...
def get_scores(args_input):
    ...
    args.records_folder = args.records_folder + f'{args.model_arch}/'
    print(args)
    ...
    save_name = records_folder + f'shift_model_{args.SUFFIX}_scores_{SEED_init}_{args.LOOP}_ref{args.model_numbs}.pth'
    torch.save(Scores, save_name)
    save_name = records_folder + f'shift_model_{args.SUFFIX}_unlearn_flags_{SEED_init}_{args.LOOP}_ref{args.model_numbs}.pth'
...
def load_pretrain_weights_shift(DEVICE, TRAIN_FROM_SCRATCH=True, RETRAIN=False, dataname='cifar10', 
                          train_loader=None, test_loader=None, 
                          checkpoints_folder="LIRA_checkpoints", SUFFIX=None, resume=False, SEED=42,
                          BREAKs=None, breakcount=0, GLIR=False, arch_name=None):
    """
    ...
    if arch_name is not None:
        model_path = model_path.split('.pth')[0] + f"arch_{arch_name}.pth"

    print(f"model_path: {model_path}")
# LIRA_checkpoints/unle_models/retrain_weights_resnet18_cifar100_cifar100_retrain_set_random_500_seed_42arch_ResNet50.pth
...
def parse_args():
    ...
    parser.add_argument('--model_arch', type=str, default='ResNet18', 
                        choices=['ResNet18', 'ResNet34', 'ResNet50', 'vgg16', 'vgg11', 'shufflenet', 'DenseNet', 'MobileNetV2', 'EfficientNet', 'swin_t'],)
    parser.add_argument('--shadow_arch', type=str, default='ResNet18', 
                            choices=['ResNet18', 'ResNet34', 'ResNet50', 'vgg16', 'vgg11', 'shufflenet', 'DenseNet', 'MobileNetV2', 'EfficientNet', 'swin_t'],) 
...
if __name__ == "__main__":
    ...
    # for dataname in ['cifar100']:
        ...
        args_input.shadow_arch = args_input.model_arch
        for arch_ in ['ResNet18', 'ResNet34', 'ResNet50', 'vgg16', 'vgg11', 'DenseNet']:
            args_input.shadow_arch = arch_
