...
class NeighborhoodAttack(AbstractAttack):
    ...
    def _generate_neighbors_for_chunk(self, tokenized_chunk):
        mlm_device = next(self.mlm_model.parameters()).device
        text_tokenized = tokenized_chunk.unsqueeze(0).to(mlm_device)
        replacements = dict()
