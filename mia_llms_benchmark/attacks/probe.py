...
def attach_hooks(model):
    ...
    if isinstance(model, torch.nn.DataParallel):
        model = model.module  
        print("DataParallel model detected. Using module attribute.")

    # For GPTNeoX models, layers are under model.gpt_neox.layers
    if hasattr(model, 'gpt_neox') and hasattr(model.gpt_neox, 'layers'):
        layers_to_hook = model.gpt_neox.layers
    # For other models like GPT2, layers might be accessed differently
    elif hasattr(model, 'transformer') and hasattr(model.transformer, 'h'):
        layers_to_hook = model.transformer.h
    # Default fallback (original code)
    elif hasattr(model, 'layers'):
        layers_to_hook = model.layers
    else:
        raise AttributeError(f"Cannot find layers in model of type {type(model).__name__}")
...
class ProbeAttack(AbstractAttack):
    ...
    def run(self, dataset: Dataset) -> Dataset:
        ...
        if hasattr(self.model, 'gpt_neox') and hasattr(self.model.gpt_neox, 'layers'):
            layers_to_hook = self.model.gpt_neox.layers
        # For other self.models like GPT2, layers might be accessed differently
        elif hasattr(self.model, 'transformer') and hasattr(self.model.transformer, 'h'):
            layers_to_hook = self.model.transformer.h
        # Default fallback (original code)
        elif hasattr(self.model, 'layers'):
            layers_to_hook = self.model.layers
        else:
            raise AttributeError(f"Cannot find layers in self.model of type {type(self.model).__name__}")
        
        handle = layers_to_hook[best_layer].register_forward_hook(hook)
    ...
    def probe_score(self, batch, probe_model, hook):
        probe_device = next(probe_model.parameters()).device
        with torch.no_grad():
            ...
            activations = hook.out[:, -1].to(probe_device)
    ...
    def collect_activations(self, proxy_model, ds_members, ds_non_members):
        ...
        labels = []
        first_device = next(proxy_model.parameters()).device

        with torch.no_grad():
            for ds, label in [(ds_members, 1), (ds_non_members, 0)]:
                for record in ds:
                    input_ids = torch.tensor(record['input_ids']).unsqueeze(0).to(first_device)
                    attention_mask = torch.tensor(record['attention_mask']).unsqueeze(0).to(first_device)
    ...
    def train_probe_classifier(self, proxy_model, ds_members, ds_non_members):
        ...
        proxy_device = next(proxy_model.parameters()).device
        ...
        for layer in activations:
            train_acts, train_labels = train_act_ds.get(layer, device=proxy_device)
            probe_model = LRProbe.from_data(train_acts, train_labels, device=proxy_device)

            dev_acts, dev_labels = dev_act_ds.get(layer, device=proxy_device)
            dev_auc = evaluate(probe_model, dev_acts, dev_labels)
            dev_auc_list.append(dev_auc)

            test_acts, test_labels = test_act_ds.get(layer, device=proxy_device)
    ...
    def fine_tune_proxy(self, ds_members):
        ...
        if os.path.exists(dirpath):
            ...
            return AutoModelForCausalLM.from_pretrained(dirpath, device_map="auto")

        logging.info("Fine-tuning proxy model")
        proxy_model = copy.deepcopy(self.model)
        if not hasattr(proxy_model.config, "model_type") or proxy_model.config.model_type is None:
            # Set this to the appropriate model type, e.g., "gpt2" or whichever is relevant.
            proxy_model.config.model_type = "gpt2"
