...
def init_model(model_name):
    ...
    if 'muse-bench' in model_name or 'muse-news' in model_name:
...
def init_dataset(ds_info, model, tokenizer, batch_size, check_path=None, step=100):
    # Load dataset based on configuration
    dataset = load_muse_books_dataset(name=ds_info["muse_name"])

    # Define file paths
    nlloss_path = f"{check_path}-{step}-nlloss.txt" if check_path else None
...
def update_dataset(dataset, model, tokenizer, batch_size, check_path=None, step=100):
    # Load dataset based on configuration
    # Define file paths
    nlloss_path = f"{check_path}-{step}-nlloss.txt" if check_path else None
    ...
    if check_path and os.path.exists(nlloss_path):
        try:
            ...
            # dataset = dataset.add_column('nlloss', cached_data)
            def update_nlloss_example(example, index, new_loss_values):
                """
                Updates the 'nlloss' field for a single example.
                """
                example['nlloss'] = new_loss_values[index]
                return example
            dataset = dataset.map(
                update_nlloss_example,
                with_indices=True,  # Pass the index to the mapping function
                fn_kwargs={'new_loss_values': cached_data} # Pass your new data to the function
            )
            logging.info(f"Loaded nlloss signals from {nlloss_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,   # Arguments are treated as strings
        help='step list to attack from 100 to 1050', # Descriptive help message
        default=[   # Set the default value as a Python list of strings
    ...
    model, tokenizer = init_model(global_config['target_model'].replace("STEP", "100"))
    ...
    results_to_print = {}
    ds_info = global_config['datasets'][0]
    ...
    # shift_model_STEPs = ['100', '150', '200', '250', '300', '350', '400', '450', '500', '550', '600', '650', '700', '750', '800', '850', '900', '950', '1000', '1050']
                        
    for step in args.STEPs:
        model, tokenizer = init_model(global_config['target_model'].replace("STEP", step))
        ...
        for attack_name in sorted(attacks, reverse=True):
            ...
            attack_config['name'] = ds_name+"-"+step
            ...
            if multiattack:
                ...
                y = [x[attack_name+"-"+step] for x in dataset]
                results_to_save[ds_name][attack_name+"-"+step] = y
                with open(output_file, 'a') as f:
                    f.write(f"{attack_name}-{step}:\n")
                    for metric in y:
                        f.write(f"{metric}\n")
                ...
                results.append([attack_name+"-"+step] + attack_results)
                logging.info(f"AUC {attack_name+"-"+step} on {ds_name}: {attack_results[0]}")
