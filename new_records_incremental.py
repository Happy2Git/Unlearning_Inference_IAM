...
def audit_nonmem_mono(args, DEVICE=torch.device("cpu"), SEED=42, batch_size=1024):
    ...
    audit_predicts = {}
    target_unl_proby = None
    if 'p_Unleak' in args.metrics:
        # train attack models on nonmem set
        attack_model = train_attack_model(
            args, model_list=args.SHADOW_IDXs, shadow_path= args.shadow_folder + 'cifar10', DEVICE=DEVICE)
    ...
    if 'p_RMIA' in args.metrics:
        ...
        p_RMIA = 1-rmia(target_unl_logits, target_shadow_logits, target_labels, 
                ...
                model_list=args.SHADOW_IDXs, metric='taylor-soft-margin', dataname= 'cifar10', DEVICE=DEVICE)
    ...
    if 'p_interapprox' in args.metrics:
        ...
        _, score_unl_weighted = interpolate_appro(target_ori_proby, target_unl_proby, shadow_target_proby, INTER_APPRO, type = 'norm')

        key = 'p_IPapprox_proby_simp'
        audit_predicts[key] = score_unl_weighted
        print(f"{key} done")

        _, _, _, extra, temperature, gamma = config_dataset('cifar10')
        ...
        _, score_unl_weighted = interpolate_appro(target_ori_tsm, target_unl_tsm, shadow_target_tsm, INTER_APPRO, type = 'norm')
        ...
        _, score_unl_weighted = interpolate_appro(target_ori_carlogi, target_unl_carlogi, shadow_target_carlogi, INTER_APPRO, type = 'norm')
        ...
        _, score_unl_weighted = interpolate_appro(-target_ori_loss, -target_unl_loss, -shadow_target_loss, INTER_APPRO, type = 'norm')
        ...
        _, score_unl_weighted = interpolate_appro(target_ori_signal_off, target_unl_logloss_off, shadow_target_logloss_off, INTER_APPRO)
...
def load_records(records_folder, args, batch_size=1024, DEVICE='cpu', SEED='42'):
    ...
    if not os.path.exists(target_ori_signal_off_path):
        ...
        shadow_fit = 0
        shadow_path= args.shadow_folder + f'{args.dataname}' if args.dataname != 'incremental' else args.shadow_folder + f'cifar10'
        for shadow_idx in args.SHADOW_IDXs:
    ...
    if RELOAD_SHADOW:
        shadow_model_list = []
        dataname = args.dataname if args.dataname != 'incremental' else 'cifar10'
...
def fetch_data_model(args,  verbose=False, SEED = 42, DEVICE='cpu'):
    ...
    # for 'incremental' dataset, init_train_loader contains 5000 cinic10 samples, retain_loader contains 4500 cinic10 samples, forget_loader contains 500 cifar10 samples
    ...
    shadow_path = f'LIRA_checkpoints/shadow_models/cifar10'
    ...
    if dataname == 'cifar10' or dataname == 'cifar100'  or dataname == 'cinic10' or dataname == 'incremental':
...
def create_shadow_lists(total_models=128, model_nums=1):
    ...
    group_nums = min(total_models // model_nums, 128)
...
def get_scores(args_input):
    ...
    for idx,shadow_list in enumerate(shadow_lists):
        ...
        for key in Scores.keys():
            ...
            # actually, the results from balanced_auc_extended are the same as the results from auc_extended
    ...
    save_name = records_folder + f'{args.SUFFIX}_scores_{SEED_init}_{args.LOOP}_ref{args.model_numbs}.pth'
    if len(args.metrics) == 1 and args.metrics[0] == 'p_interapprox':
        save_name = save_name.replace('.pth', '_onlyinterapprox.pth')
    torch.save(Scores, save_name)
    save_name = records_folder + f'{args.SUFFIX}_unlearn_flags_{SEED_init}_{args.LOOP}_ref{args.model_numbs}.pth'
    if len(args.metrics) == 1 and args.metrics[0] == 'p_interapprox':
        save_name = save_name.replace('.pth', '_onlyinterapprox.pth')
...
def parse_args():
    ...
    parser.add_argument('--dataname', type=str, default=None, choices=['cifar10', 'cifar100', 'cinic10', 'texas', 'purchase', 'location', 'incremental'],
                        help="Dataset to use")
    ...
    parser.add_argument('--metrics', nargs='+', default=['p_Unleak', 'p_LiRA', 'p_update_LiRA', 'p_EMIA', 'p_EMIA_p', 'p_RMIA', 'p_interapprox', 'p_random'],
...
# start_print
# Namespace(dataname='incremental', unlearn_method='retrain', unlearn_type='set_random', forget_class=0, forget_size=500, forget_class_ratio=0.5, model_numbs=1, SHADOW_AVE_FLAG=False, LOOP=1, EXACT_FLAG=False, shadow_folder='LIRA_checkpoints/shadow_models/', records_folder='/ibex/project/c2283/Evaluate-Unlearning/records/', SEED_init=42, CLASS_init=0, INTERAPPROX=100, metrics=['p_Unleak', 'p_LiRA', 'p_update_LiRA', 'p_EMIA', 'p_EMIA_p', 'p_RMIA', 'p_interapprox'], MODEL_CHECK=False, SHADOW_IDXs=[0], SUFFIX='incremental_retrain_set_random_500')
# dict_keys(['p_Unleak', 'p_LiRA', 'p_LiRA_Online', 'p_update_LiRA', 'p_EMIA', 'p_EMIA_p', 'p_RMIA', 'p_RMIA_online', 'p_IPapprox_proby_simp', 'p_IPapprox_tsm_simp', 'p_IPapprox_carlogi_simp', 'p_IPapprox_losy_simp', 'p_IPapprox_simp_online', 'p_IPapprox_simp_offline', 'p_IPapprox_simp_online_wo_norm'])
# Metric values                  auc    acc    tpr0.0 .-5    .-2    tnr0.0 .-5    .-2    1-mean 1-std  0-mean 0-std  cm-std1 cm-std0
# p_Unleak                       0.8633 0.8220 0.0318 0.0318 0.1000 0.0000 0.0111 0.6420 0.8144 0.2564 0.4272 0.1401
# p_LiRA                         0.6172 0.6064 0.0080 0.0080 0.1013 0.0000 0.00e+00 1.80e-02 -1.36e+61 2.10e+61 -3.08e+61 4.02e+61
# p_LiRA_Online                  0.8206 0.7390 0.0769 0.0769 0.1889 0.0000 8.46e-03 3.78e-01 2.09e+61 4.29e+61 -2.70e+61 4.00e+61
# p_update_LiRA                  0.7014 0.6934 0.0000 0.0000 0.0261 0.0000 0.0001 0.0997 0.4000 0.5477 -0.0351 0.3466
# p_EMIA                         0.8326 0.7538 0.0113 0.0113 0.2313 0.2980 0.2980 0.3925 0.2728 0.2176 0.0534 0.1165
# p_EMIA_p                       0.8240 0.7980 0.0042 0.0042 0.0153 0.0400 0.0569 0.5480 0.5979 0.2686 0.2890 0.1292
# p_RMIA                         0.8686 0.7990 0.0291 0.0291 0.2187 0.1700 0.1841 0.5780 0.7547 0.1498 0.4940 0.2177
# p_RMIA_online                  0.8816 0.8068 0.0000 0.0000 0.2120 0.1540 0.1699 0.6000 0.8046 0.1209 0.6217 0.1267
# p_IPapprox_proby_simp          0.8602 0.8028 0.0000 0.0000 0.2038 0.5620 0.5620 0.5993 0.6959 0.2756 0.3316 0.2079
# p_IPapprox_tsm_simp            0.8916 0.8113 0.0169 0.0169 0.1660 0.3100 0.3100 0.5984 0.7553 0.2715 0.2890 0.2065
# p_IPapprox_carlogi_simp        0.8745 0.8032 0.0360 0.0360 0.0980 0.3020 0.3020 0.4954 0.7439 0.2769 0.3025 0.2272
# p_IPapprox_losy_simp           0.8358 0.7891 0.0013 0.0013 0.0862 0.5380 0.5380 0.5778 0.6464 0.2574 0.3704 0.1919
# p_IPapprox_simp_online         0.8922 0.8172 0.0016 0.0016 0.2004 0.5340 0.5340 0.6204 0.7297 0.2853 0.2690 0.1647
# p_IPapprox_simp_offline        0.8964 0.8244 0.0813 0.0813 0.3138 0.3480 0.3480 0.5965 0.7269 0.2947 0.2449 0.1698
# p_IPapprox_simp_online_wo_norm 0.8922 0.8172 0.0016 0.0016 0.2004 0.5340 0.5340 0.6204 0.7297 0.2853 0.2690 0.1647
# end_print