...
def get_scores(args_input):
    ...
    for idx,shadow_list in enumerate(shadow_lists):
        ...
        for loop in range(args.LOOP):
            ...
            Member_scores = []
            Scores = {}
            save_name = records_folder + f'{args.SUFFIX}_internal_shadow_{args.select_idx}_shift_ref_{args.model_numbs}'+'_'.join(map(str, shadow_list))+'.pth'
...
def parse_args():
    ...
    parser.add_argument('--records_folder',type=str, default='LIRA_checkpoints/Evaluate-Unlearning/records/')
...
if __name__ == "__main__":
    ...
    if None == args_input.dataname:
        for dataname in ['cifar10', 'cinic10']:
